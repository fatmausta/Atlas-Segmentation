#use 15 atlases and 2 targets

#initially regixtered images, by centering, used a new directory for the data and teh results


#DOWNSAMPLED DATA
 #OVERLAPPING DATA WITH THE FISRT DATASET


# 11 atlases, 10 targets use old atlases primary an dadd from the new atlases

#exlude badly cropped atlases
#smooth myos

#used smooth myo

#change the target to 1115 (largets atlas)

#aff+def for LGEtoMRA
...
data_root = '/home/fusta/Dropbox/CODING/UBUNTU/git/Multi-Atlas-Segmentation/Registration-Pipeline/data/norm_iso_scaled_rigid/'
proc_dump = '/home/fusta/Dropbox/CODING/UBUNTU/git/Multi-Atlas-Segmentation/Registration-Pipeline/tmp/norm_iso_scaled_rigid/'
...
#excluded atlases: 0515, 0546, 0565, 1115
#randomly deleted extra atlases: 0493, 0574, 1076

atlas_dataset = ['0364', '0485', '0495', '0519', '0529', '0574', '0578', '0715', '1166', '1168', '1171', '0417', '0424', '0473', '0493', '0591']
target_dataset = ['0485', '0364']#, '0495', '0519', '0529', '0574', '0578', '0715', '1166', '1168', '1171', '0417', '0424', '0473', '0493', '0591', '0601', '0632', '0730', '0917', '0921', '1073', '1076']
start = timer()
...
for target_id in target_dataset: #target dataset for cross_correlation, if no cross-correlation, uses only one target
    ...
    for a in atlas_dataset:
        ...
        if LGEtoMRA_registerResampleFlag == True:
            ...
            #def resample
            flo_lbl2 = res_lbl3
            def_lbl2 = os.path.join(data_root, a, 'myo' + a + 'forMRA_aff_def.nii')
    ...
    if affResampleLabelFLag == True:
        for a in atlas_dataset:
            ...
            if def_reg_LGEtoMRA==True:
                flo_lbl2 = os.path.join(data_root, a, 'myo' + a + 'forMRA_aff_def.nii')
            else:
    ...
    if MRAtoLGE_registerResampleFlag == True:
        ...
        #flo_lbl1 = os.path.join(data_root, target_id, 'LGE' + target_id + '.nii')
        #Affine registered result will be resampled without deformable
        #flo_lbl2 = os.path.join(proc_dump, target_id, 'MRAtoLGE', aff_reg_type_MRAtoLGE,  'myo_result_iso_norm_be' + be + '-le' + le2 + '-alpha' + alpha + '-beta' + beta + '-rp' + rp2 + '-rs' + rs2 + target_id + '.nii ')
        #global could be either affine or def, incase of affine, flo_lbl_2 will be overwritten
